...
function prepend(what, where) {
    ...
    first ? where.insertBefore(what, first) : where.appendChild(what);
}
...
function collectDOMStat(root) {
    ...
    const collectTags = (node) => {
        ...
        tags[tag] = 1 + (tags.hasOwnProperty(tag) ? tags[tag] : 0);
    }
    ...
    const collectClasses = (node) => {
        ...
        classList.forEach((className) => {
            classes[className] = 1 + (classes.hasOwnProperty(className) ? classes[className] : 0);
        });
    }
